...
export default async function main(ui: any, t = (s: string) => s) {
  ...
  function renderQrcode(ui: any, t: (t: string) => string) {
    ui.form(
      ...
      async ({ values }: any) => {
        const { tableId, urlFieldId, attachmentFieldId, qrcodeType, width, checkbox } = values;
        ...
        ui.showLoading(' ');
        const totalCellValueCount = valueList.length;
        let count = 0;
  ...
  function renderBarcode(ui: any, t: (t: string) => string) {
    ui.form(
      ...
      async ({ values }: any) => {
        const { tableId, urlFieldId, attachmentFieldId, barcodeType = 'png', width, height, text, ...rest } = values;
        ...
        try {
          for (let i = 0; i < recordList.length; i++) {
            ...
            console.log(recordList[i], 'url', url)
            let size, type, barcodeFile, fileName;
            if (barcodeType == 'svg') {
